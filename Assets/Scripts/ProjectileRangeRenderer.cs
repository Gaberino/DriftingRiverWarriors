...
public class ProjectileRangeRenderer : MonoBehaviour {
	...
	void Update () {
		if (!myProj.fired){
			...
			myLR.SetPosition(1, (this.transform.position + this.transform.up * this.transform.localScale.y / 2) + (Vector3)myProj.velocity * myProj.life * 60); //this ends where the arrow will fall
		}
		else {
